  ...
  const connect = async () => {
    try {
      ...
      if (MAX_RETRIES && retryCount >= MAX_RETRIES) {
        logger.error(`Max retries (${MAX_RETRIES}) reached. Giving up on Redis connection.`);
        return;
      }
      ...
      logger.info(`Retrying Redis connection in ${delay / 1000} seconds (attempt ${retryCount})...`);
