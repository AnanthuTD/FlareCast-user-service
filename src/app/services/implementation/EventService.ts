...
export class EventService implements IEventService {
	...
	async publishUserVerifiedEvent(eventData: UserVerifiedEvent): Promise<void> {
		try {
			...
			logger.debug(
				`✔️ Published user.verified event: ${JSON.stringify(
					eventData,
					null,
					2
				)}`
			);
			this.publishSubscriptionUpdateEvent({ userId: eventData.userId });
		} catch (error: any) {
			logger.error(
			...
			throw new Error(
				`Failed to publish user.verified event: ${error.message}`
			);
		}
	}
	...
	async publishSubscriptionUpdateEvent({
		userId,
	}: {
		userId: string;
	}): Promise<void> {
		try {
			const plan = await this.userSubscriptionRepository.getActivePlan(userId);

			console.log("plan: ", plan);

			if (plan) {
				const eventData = {
					hasAdvancedEditing: plan.hasAdvancedEditing,
					hasAiFeatures: plan.hasAiFeatures,
					maxMembers: plan.maxMembers,
					maxRecordingDuration: plan.maxRecordingDuration,
					maxVideoCount: plan.maxVideoCount,
			...
			logger.error(
				`Failed to publish ${TOPICS.USER_SUBSCRIPTION_UPDATED} event`,
				error
			);
			throw new Error(
				`Failed to publish ${TOPICS.USER_SUBSCRIPTION_UPDATED} event: ${error.message}`
			);
		}
	}
}
