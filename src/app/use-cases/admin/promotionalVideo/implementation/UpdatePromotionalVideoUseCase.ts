...
@injectable()
export class UpdatePromotionalVideoUseCase
	implements IUpdatePromotionalVideoUseCase
{
	constructor(
	...
	async execute(dto: UpdatePromotionalVideoDTO): Promise<
		ResponseDTO & {
			data: UpdatePromotionalVideoResponseDTO | { error: string };
		}
	> {
		try {
			...
			axios
				.patch(
					`${env.VIDEO_SERVICE}/api/interservice/video/${video.videoId}/visibility`,
					{ visibility: !dto.hidden }
				)
				.catch((e) => console.error("Failed to update video in video service!"));
